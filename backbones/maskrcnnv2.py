...
class MaskRCNNV2(BackboneModel):
    extracted_features = None

    def __init__(self, filepath=None):
        super().__init__(filepath=filepath)
        self.name = "maskrcnnv2"
        self.feature_keys = [f"f{i}" for i in range(1,5)]
        self.model = models.detection.maskrcnn_resnet50_fpn_v2(weights=models.detection.MaskRCNN_ResNet50_FPN_V2_Weights.DEFAULT)
    ...
    def get_features(self, input_batch, key):
        ...
        def hook_feat_map(mod, inp, out):
            MaskRCNNV2.extracted_features = out
        ...
        with torch.no_grad():
            ...
            handle.remove()
            return MaskRCNNV2.extracted_features
