...
class KeyPointRCNN(BackboneModel):
    extracted_features = None

    def __init__(self, filepath=None):
        super().__init__(filepath=filepath)
        self.name = "keypoint_rcnn"
        self.feature_keys = [f"f{i}" for i in range(1,5)]
        self.model = models.detection.keypointrcnn_resnet50_fpn(weights=models.detection.KeypointRCNN_ResNet50_FPN_Weights.DEFAULT)
    ...
    def get_features(self, input_batch, key):
        ...
        def hook_feat_map(mod, inp, out):
            KeyPointRCNN.extracted_features = out
        ...
        with torch.no_grad():
            ...
            handle.remove()
            return KeyPointRCNN.extracted_features
